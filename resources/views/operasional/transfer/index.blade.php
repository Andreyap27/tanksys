...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1.25rem;margin-bottom:1.5rem;">
    <div class="dash-stat ds-stock" style="border-radius:var(--radius-lg,.75rem);padding:1.25rem;position:relative;overflow:hidden;">
        <div class="dash-stat__header">
            <div class="dash-stat__icon"><i data-lucide="arrow-right-left" style="width:20px;height:20px;"></i></div>
            <div>
                <div class="dash-stat__label">Total Transfer</div>
                <div class="dash-stat__value" id="transferTotal_all">0 L</div>
            </div>
        </div>
        <div class="dash-stat__bg-icon"><i data-lucide="arrow-right-left" style="width:110px;height:110px;"></i></div>
    </div>
...
@php $txSection='stock-transfer'; $txHasKapal=true; $txHasMobil=false; $txHasPc=false; @endphp
...
function updateSummary(api) {
    ...
    document.getElementById('transferTotal_all').textContent = fmt(total) + ' L';
    ['biru','kuning'].forEach(w => {
        document.getElementById(`transferTotal_${w}`).textContent = fmt(totals[w]) + ' L';
    });
}
