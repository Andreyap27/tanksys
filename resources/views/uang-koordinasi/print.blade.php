...
<head>
    ...
    <title>Laporan Uang Koordinasi</title>
    <style>
...
@php
    ...
    $totalNominal = 0; $totalExtra = 0; $totalAll = 0;
    foreach ($items as $item) {
        $totalNominal += (float)$item->nominal;
        $totalExtra   += (float)$item->extra;
        $totalAll     += (float)$item->total;
    }
    $fromLabel = $from ? \Carbon\Carbon::parse($from)->translatedFormat('d M Y') : '-';
    $toLabel   = $to   ? \Carbon\Carbon::parse($to)->translatedFormat('d M Y')   : '-';
@endphp
...
<div class="page">
    ...
    <div class="rpt-label">
        <span class="rpt-label-title">Laporan Uang Koordinasi</span>
        <span class="rpt-label-sub">
            {{ $fromLabel }} — {{ $toLabel }}
            @if($kategori) &nbsp;|&nbsp; {{ $kategoriBiayaLabel[$kategori] ?? $kategori }} @endif
        ...
        <table class="rpt-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $item->nama }}</td>
                    <td>{{ $item->jabatan ?? '-' }}</td>
                    <td>{{ $item->kategori_biaya ? ($kategoriBiayaLabel[$item->kategori_biaya] ?? $item->kategori_biaya) : '-' }}</td>
                    <td class="r">Rp {{ $fmt($item->nominal) }}</td>
                    <td class="r">{{ $item->extra > 0 ? 'Rp '.$fmt($item->extra) : '-' }}</td>
                    <td class="r">Rp {{ $fmt($item->total) }}</td>
                </tr>
                @empty
                <tr><td colspan="9" style="text-align:center;padding:2rem;color:#888;font-style:italic;">Tidak ada data</td></tr>
                @endforelse
            </tbody>
            @if($items->count() > 0)
            <tfoot>
                <tr>
                    <td colspan="5">TOTAL ({{ $items->count() }} transaksi)</td>
                    <td class="r">Rp {{ $fmt($totalNominal) }}</td>
                    <td class="r">Rp {{ $fmt($totalExtra) }}</td>
                    <td class="r">Rp {{ $fmt($totalAll) }}</td>
                </tr>
            </tfoot>
            @endif
        </table>
